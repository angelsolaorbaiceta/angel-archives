...
func TestCreateArchive(t *testing.T) {
	...
	t.Run("archive header first file entry", func(t *testing.T) {
		...
		want := &HeaderFileEntry{
			...
			Size:   uint32(fileOne.SizeInBytes()),
		}
	...
	t.Run("archive header second file entry", func(t *testing.T) {
		...
		want := &HeaderFileEntry{
			...
			Offset: wantHeaderLen + 1 + uint32(fileOne.SizeInBytes()),
			Size:   uint32(fileTwo.SizeInBytes()),
		}
...
// an ArchiveFile with the file's name and expected raw bytes.
func createTempFileForTest(t *testing.T, fileName, content string) *ArchiveFile {
	...
	return NewFileFromData(data, filePath)
}
